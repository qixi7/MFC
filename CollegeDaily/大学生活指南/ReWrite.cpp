...
BOOL CReWrite::OnInitDialog()
{
	...
	CMyFriend *p = (CMyFriend*)this->GetParent();                               //获得主窗口应用实例
	CString TargetName = p->m_Tree.GetItemText(p->m_Tree.GetSelectedItem());    //保存选中好友的文本信息(姓名)
	CString sql_DDL;
...
{
	// TODO: 在此添加控件通知处理程序代码
	CMyFriend *p = (CMyFriend*)this->GetParent();			   //获取父窗口应用实例
	CString TargetName = p->m_Tree.GetItemText(p->m_Tree.GetSelectedItem());  //用于保存选中的好友名
	...
	try
	{
		...
		//就无法返回结果集的条数，只能返回－1
		if (sql.m_pReset->GetRecordCount() != 0 && sql.m_pReset->GetRecordCount() != 1)//判断sql语句所得记录集是否有记录
		{
