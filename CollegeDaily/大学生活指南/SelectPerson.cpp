...
BEGIN_MESSAGE_MAP(CSelectPerson, CDialogEx)
	ON_BN_CLICKED(IDC_FIND, &CSelectPerson::OnClickedFind)
END_MESSAGE_MAP()
...
void CSelectPerson::OnClickedFind()
{
	// TODO: 在此添加控件通知处理程序代码
	...
	try
	{
		...
		p->m_List.DeleteAllItems();                                                //清空详细信息列表(ListControl)
		while (!sql.m_pReset->adoEOF)											//若记录集指针不超出末尾则执行循环
		{
			p->m_List.InsertItem(0,_T(""));                                                      //插入行
			p->m_List.SetItemText(0,0,(LPCTSTR)(_bstr_t)sql.m_pReset->GetCollect("姓名"));	 //设置姓名列信息
			p->m_List.SetItemText(0,1,(LPCTSTR)(_bstr_t)sql.m_pReset->GetCollect("性别"));	 //设置性别列信息
			p->m_List.SetItemText(0,2,(LPCTSTR)(_bstr_t)sql.m_pReset->GetCollect("电话"));	 //设置电话列信息
			p->m_List.SetItemText(0,3,(LPCTSTR)(_bstr_t)sql.m_pReset->GetCollect("邮箱"));	 //设置邮箱列信息
			p->m_List.SetItemText(0,4,(LPCTSTR)(_bstr_t)sql.m_pReset->GetCollect("所在分组")); //设置所在分组列信息
